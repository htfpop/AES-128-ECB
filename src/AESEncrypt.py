...
def s_box_sub(byte):
    ...
    ls_nibble = (byte & 0x0F)
    print(f'MS Nibble = {hex(ms_nibble)} , LS Nibble = {hex(ls_nibble)} \n')
    #   print(f'S-Box Location = 0x{SBOX[MSNibble][LSNibble]:02x}\n')
    return SBOX[ms_nibble][ls_nibble]


def test():
    start = 0x00
    check = [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
             0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
             0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
             0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,
             0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84,
    ...
    for x in range(0x100):
        ...
        if val != check[start]:
            print(f'[ERROR]: Mismatch between start value of 0x{start:02X} and SBOX value of 0x{check[start]:02X}\r\n')
...
def mixColsTest():
    ...
    if tools.compare_2d(state, KAT0) == 0:
    ...
    if tools.compare_2d(state, KAT1) == 0:
    ...
    if tools.compare_2d(state, KAT2) == 0:
    ...
    if tools.compare_2d(state, KAT3) == 0:
    ...
    if tools.compare_2d(state, KAT4) == 0:
    ...
    if tools.compare_2d(state, KAT5) == 0:
...
"""
    ...
    mxcoltest0 = [0xd4, 0xbf, 0x5d, 0x30]
